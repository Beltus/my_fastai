...
for drop_base in [0.001, 0.01, 0.02, 0.05, 0.1, 0.5]:
    prefix = f'all_drop_{str(drop_base).replace(".","pt")}'
    dropouti=drop_base
    dropout=drop_base
    wdrop=drop_base
    dropoute=drop_base
    dropouth=drop_base
    for drop in ['di', 'd', 'wd', 'de', 'dh']:
        for i in range(range_strt, range_stop):
            drop_val = i / 20
            run_id = f'{prefix}_{drop}_{str(drop_val).replace(".","pt")}'
            ...
            dps = np.array([dropouti, dropout, wdrop, dropoute, dropouth]) * drops_scalar

            learner= md.get_model(opt_fn, em_sz, nh, nl,
                dropouti=dps[0], dropout=dps[1], wdrop=dps[2], dropoute=dps[3], dropouth=dps[4])
            ...
            learner.save('lm_last_ft'+'_'+str(run_id))

            learner.load('lm_last_ft'+'_'+str(run_id))
            ...
            save_obj(ep_vals, 'imdb_ep_vals_full_'+run_id)
